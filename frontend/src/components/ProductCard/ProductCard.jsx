...
/**
 * Displays product information with optional loading state
 */
const ProductCard = ({ title, value, extra, color, icon, loading }) => {
  return (
    <div className="ProductCard">
      {/* Product textual information */}
      <div className="product-informations-card">
        <p className="product-title-card">
          {loading ? <Skeleton width={100} /> : title}
        </p>

        <p className="product-value-card">
      ...
      <div className={`product-icon-card ${color}`}>
        {loading ? (
          <Skeleton square borderRadius={10} width={48} height={48} />
        ) : (
          icon
        )}
      </div>
